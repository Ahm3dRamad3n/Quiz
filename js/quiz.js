  ...
  apiFetch("/auth/delete-account", { method: "POST" })
    ...
    .catch((error) => {
      if (error && error.code === "auth/requires-recent-login") {
        alert(
          "Security restriction: Please sign out and sign back in before deleting your account.",
        );
        return;
      }
      handleFirestoreErrorGlobal(error, "Failed to delete account completely.");
    });
}
...
function displayQuizzes() {
  ...
  if (landingMode) {
    ...
    const ctaL = document.createElement("button");
...
function updateQuizExpiryAt() {
  ...
  const oldExpiryDate = new Date(currentQuiz.expireAt);
  ...
  if (diffInDays >= 1) {
    apiFetch(`/quizzes/${currentQuiz.id}/extend`, { method: "POST" })
      .then((res) => {
        if (res.ok) {
          console.log("Quiz expiration extended successfully via API.");
        }
      })
      .catch((e) => console.log("Ignore extension noise:", e));
  }
}
...
function bindAllEvents() {
  ...
  document
    .getElementById("toggleUserDropdown")
    ?.addEventListener("click", toggleUserDropdown);
  document
    .getElementById("settingsBtn")
    ?.addEventListener("click", openSettingsModal);

  // 2. أزرار الـ Settings Modal
  document
    .getElementById("closeSettingsModalBtn")
    ?.addEventListener("click", closeSettingsModal);

  // إغلاق نافذة الإعدادات عند الضغط خارجها
  const settingsOverlay = document.getElementById("settingsModalOverlay");
  settingsOverlay?.addEventListener("click", (e) => {
    // نتأكد إن الضغطة كانت على الخلفية الشفافة مش على المحتوى الأبيض
    if (e.target === settingsOverlay) {
      closeSettingsModal();
    }
  });

  // 3. أزرار الـ PDF Modal
  document
    .getElementById("closePdfModalBtn")
    ?.addEventListener("click", closeQuizPdfModal);
  document
    .getElementById("pdfAnsweredBtn")
    ?.addEventListener("click", () => printQuizAs("answered"));
  document
    .getElementById("pdfUnansweredBtn")
    ?.addEventListener("click", () => printQuizAs("unanswered"));

  // تغيير اللغة في الـ PDF
  document.getElementById("pdfLangEn")?.addEventListener("change", () => {
    const overlay = document.getElementById("pdfModalOverlay");
    if (overlay) overlay.dataset.selectedLang = "en";
    document.getElementById("pdfAnsweredBtn").disabled = false;
    document.getElementById("pdfUnansweredBtn").disabled = false;
  });

  document.getElementById("pdfLangAr")?.addEventListener("change", () => {
    const overlay = document.getElementById("pdfModalOverlay");
    if (overlay) overlay.dataset.selectedLang = "ar";
    document.getElementById("pdfAnsweredBtn").disabled = false;
    document.getElementById("pdfUnansweredBtn").disabled = false;
  });
  ...
  document
    ...
    ?.addEventListener("click", toggleTranslate);
  document.getElementById("goHomeBtn1")?.addEventListener("click", goHome);
  document
  ...
  // 5. أزرار صفحة النتائج
  document
    .getElementById("langToggle")
    ?.addEventListener("change", toggleTranslate);
  document
    .getElementById("downloadPdfBtn")
    ?.addEventListener("click", downloadErrorsPDF);
